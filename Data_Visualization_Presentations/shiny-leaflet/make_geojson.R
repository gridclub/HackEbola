...
transdat <- spTransform(dat, CRS("+init=epsg:4326"))

# ----- change name of field we will map
# names(transdat)[names(transdat) == "DP0010001"]<-"Population"
...
transdat <- SpatialPolygonsDataFrame(transdat, data=transdat@data)
...
writeOGR(obj=transdat, dsn=leafdat, layer="", driver="GeoJSON")
